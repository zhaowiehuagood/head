...
//                             let $ = cheerio.load(body);
//                             let time = $(".article-aboute span").eq(0).text();
//                             let concent = $("#article-content").html();
//                             let insert_sql = "insert into news (title,concent) values (?,?)";
//                             connection.query(insert_sql, [title, concent], function (error, result, fields) {
//                                 if (error) throw error;
//                             })
//                         }
//                     )
...
//获取中关村新闻列表
request({
    url:"https://m.zol.com.cn/",
    encoding:null,
},function(err,res,body){
    body=iconv.decode(body,"gb2312");
    let $=cheerio.load(body);
    let s=$(".swiper-wrapper li").text();
    console.log(s);
    let insert_sql = "insert into news (name,con) values (?,?)";
    connection.query(insert_sql, ["",s], function (error, result, fields) {
